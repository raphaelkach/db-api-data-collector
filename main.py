...
def extract_change_data(s):
    return {
        ...
        "departure_status": (
            s.find("dp").get("cs") if s.find("dp") is not None else None
        ),
    }
        ...
        for i, eva_nr in enumerate(eva_nrs):
            ...
            if response_plan and response_change:
                ...
                if plan_root and change_root:
                    plan_df, change_df = process_data(
                        plan_root, change_root, eva_nr, metadata
                    )
                    all_plan_data.append(plan_df)
                    all_change_data.append(change_df)
                else:
                    logging.warning(
                        f"XML-Daten konnten für EVA-Nr. {eva_nr} nicht geparst werden."
                    )
            else:
                logging.warning(
                    f"API-Daten konnten für EVA-Nr. {eva_nr} nicht abgerufen werden."
                )
