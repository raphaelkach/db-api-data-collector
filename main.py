...
STATIONS_FILEPATH = os.path.join(BASE_DIR, '01_data', '01_raw', 'Stationen_S-Bahnnetz_Stuttgart.csv')
...
def save_data(plan_df, change_df, timestamp):
    merged_df = pd.merge(plan_df, change_df, how='left', on='ID', suffixes=('_plan', '_change'))
    ...
    if (merged_df['arrival_delay_m'].isna().all() or merged_df['arrival_delay_m'].eq(0).all()) and \
       (merged_df['departure_delay_m'].isna().all() or merged_df['departure_delay_m'].eq(0).all()):
        logging.info(f"Keine gültigen Verspätungsdaten für {timestamp}, Speichern übersprungen.")
        return
    ...
    # Überprüfen, ob eine alte CSV existiert, bevor sie überschrieben wird
    merged_file = os.path.join(OUTPUT_DIR, f"{timestamp}.csv")
    if os.path.exists(merged_file):
        old_data = pd.read_csv(merged_file)
        if not old_data.empty:
            logging.info(f"Gültige Daten bereits vorhanden, Datei für {timestamp} wird nicht überschrieben.")
            return

    # Speichern der gemergten Daten
...
def extract_plan_data(s, eva_nr, metadata):
    return {
        ...
        'planned_departure': s.find('dp').get('pt') if s.find('dp') is not None else None,
        'planned_platform': s.find('ar').get('pp') if s.find('ar') is not None else None,
...
def extract_change_data(s):
    return {
        ...
        'changed_platform_arrival': s.find('ar').get('cp') if s.find('ar') is not None else None,
        'changed_departure': s.find('dp').get('ct') if s.find('dp') is not None else None,
        'changed_platform_departure': s.find('dp').get('cp') if s.find('dp') is not None else None,
        ...
        'departure_hidden': s.find('dp').get('hi') if s.find('dp') is not None else None,
        'arrival_path': s.find('ar').get('ppth') if s.find('ar') is not None else None,
        'departure_path': s.find('dp').get('ppth') if s.find('dp') is not None else None,
